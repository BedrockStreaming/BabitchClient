...
describe('Babitch : Game', function() {
	...
	it('should cancel last goal',function() {
		...
		expect(element('.action .btn-group ul li:eq(0) a').css('display')).toBe('block');
		expect(element('.action .btn-group ul li:eq(0) a').text()).toMatch('Cancel last goal');

		//Cancel the goal
		element('.action .btn-group ul li:eq(0) a').click();
  		expect(element('.score strong:eq(0)').text()).toBe('0');
  		expect(element('.score strong:eq(1)').text()).toBe('0');

  		//The button must be hidden
		expect(element('.action .btn-group ul li:eq(0)').css('display')).toBe('none');
	});
